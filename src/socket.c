 ...
 * Usually, it is better to use osmo_sock_get_name2() for a static string buffer or osmo_sock_get_name2_buf() for a
...
char *osmo_sock_get_name(void *ctx, int fd)
{
	...
	int rc;
	rc = osmo_sock_get_name2_buf(str, sizeof(str), fd);
	if (rc <= 0)
		return NULL;
...
const char *osmo_sock_get_name2(int fd)
{
	...
	osmo_sock_get_name2_buf(str, sizeof(str), fd);
	return str;
}
